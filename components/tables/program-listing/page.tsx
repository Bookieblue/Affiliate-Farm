...
const AdsPage = () => {
  ...
  const { mutate, isSuccess: deleteSuccess } =
    useDeleteProgram(selectedDeletes);
  ...
  const handleConfirmEdit = () => {
    ...
    const selectedRow = data?.find((row) => row.code === selectedRows[0]);
    if (selectedRow && editCategory) {
